...
$konfigurasi['d'] 			= "pegawai2";			// db name 
$konfigurasi['pengguna']	= "smk";			// tingkat pengguna pilihan = "smk" atau "smp"



mysqli_connect($konfigurasi['h'], $konfigurasi['u'], $konfigurasi['p']) or die (mysqli_error());
mysqli_select_db($konfigurasi['d']);

function getSekolah() {
	$q = mysqli_query("SELECT * FROM t_sekolah WHERE flag = '1'");
...
function cmbDB($name, $tabel, $f_value, $f_view, $selected) {
	...
	$q = mysqli_query($connect,"SELECT $f_value, $f_view FROM $tabel ORDER BY $f_value ASC");
...
function cCmbPekerjaan($val1, $val2) {
	...
	$q = mysqli_query($connect,"SELECT * FROM t_pkj ORDER BY pkj ASC");
	while ($a = mysqli_fetch_array($q)) {
		if ($a[0] == $val1) {
			echo "<option value='$a[0]' selected>$a[1]</option>";
		} else {
	...
	$r = mysqli_query($connect,"SELECT * FROM t_pkj ORDER BY pkj ASC");
...
function cCmbPendidikan($val1, $val2) {
	...
	$q = mysqli_query($connect,"SELECT * FROM t_penddk ORDER BY id_penddk ASC");
	while ($a = mysqli_fetch_array($q)) {
		if ($a[0] == $val1) {
			echo "<option value='$a[0]' selected>$a[1]</option>";
		} else {
	...
	$r = mysqli_query($connect,"SELECT * FROM t_penddk ORDER BY id_penddk ASC");
	while ($b = mysqli_fetch_array($r)) {
		if ($b[0] == $val2) {
			echo "<option value='$b[0]' selected>$b[1]</option>";
		} else {
...
function cPrestasi($val_nama, $val_tkt) {
	...
	for ($i=1; $i<=3; $i++) {
		...
		$q = mysqli_query($connect,"SELECT * FROM t_prestasi ORDER BY id_prestasi ASC");
...
function cPilihanJurusan($val) {
	...
	for ($i=1; $i<=2; $i++) {
		...
		$q = mysqli_query($connect,"SELECT * FROM t_jurusan ORDER BY id_jur ASC");
		while ($a = mysqli_fetch_array($q)) {
			$idx = $i-1;
			if ($pc_value[$idx] == $a[0]) {
				echo "<option value='$a[0]' selected>$a[1]</option>";	
...
function getAgama($id_agama) {
	$q = mysqli_query($connect,"SELECT * FROM data_pegawai WHERE Agama='$id_agama'");
	$a = mysqli_fetch_array($q);
	return $a[1];
}
function getPendidikan($id_pddk) {
	$q = mysqli_query($connect,"SELECT * FROM t_penddk WHERE id_penddk='$id_pddk'");
	$a = mysqli_fetch_array($q);
	return $a[1];
}

function getJabatan($id_jab) {
	$q = mysqli_query($connect,"SELECT * FROM t_jab WHERE id_jab='$id_jab'");
	$a = mysqli_fetch_array($q);
	return $a[1];
}

function getPekerjaan($id_pkj) {
	$q = mysqli_query($connect,"SELECT * FROM t_pkj WHERE id_pkj='$id_pkj'");
	$a = mysqli_fetch_array($q);
	return $a[1];
}
function getTktPrestasi($id_pres) {
	$q = mysqli_query($connect,"SELECT * FROM t_prestasi WHERE id_prestasi='$id_pres'");
	$a = mysqli_fetch_array($q);
	return $a[1];
}
function getJurusan($id_jur) {
	$q = mysqli_query($connect,"SELECT * FROM t_jurusan WHERE id_jur='$id_jur'");
	$a = mysqli_fetch_array($q);
	return $a[1];
}

function getmax($p_noreg) {
	$q = mysqli_query($connect,"SELECT max($p_noreg) from master");
	$a = mysqli_fetch_array($q);
	return $a[1];
}
...
// or print('Error performing query \'<strong>' . $templine . '\': ' . mysqli_error() . '<br /><br />')
...
function getValue($tabel, $field, $fk, $id) {
	...
	$q = mysqli_query("SELECT $field FROM $tabel $where");
	$a = mysqli_fetch_array($q);
	return $a[0];
