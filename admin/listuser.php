...
?>
 ...
 <div class="span12">
	  <?php 
        ...
        $jml_data = mysqli_num_rows(mysqli_query("select id_user,username,w_login,photo,user.n_lv, status_app.approve from tbl_user,user,status_app where tbl_user.level_user=user.level_user and tbl_user.kd_approve=status_app.kd_approve order by w_login"));
        ...
        if(isset($_POST['cari'])) {
            ...
            $query = mysqli_query($connect,$connect,"
        ...
        // query jika nomor halaman sudah ditentukan
            ...
            $query = mysqli_query($connect,"select id_user,username,w_login,photo,user.n_lv, status_app.approve from tbl_user,user,status_app where tbl_user.level_user=user.level_user and tbl_user.kd_approve=status_app.kd_approve order by w_login desc LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
        // query ketika tidak ada parameter halaman maupun pencarian
        } else {
            $query = mysqli_query($connect,"select id_user,username,w_login,photo,user.n_lv, status_app.approve from tbl_user,user,status_app where tbl_user.level_user=user.level_user and tbl_user.kd_approve=status_app.kd_approve order by w_login LIMIT 0, $jml_per_halaman");
            $halaman = 1; //tambahan
        }
