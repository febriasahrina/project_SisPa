...
$mod 		= isset($_GET['mod']) ? $_GET['mod'] : NULL;	
		...
		if ($mod == "del") {
			$q_delete_agama = mysqli_query($connect,"DELETE FROM tbl_bank WHERE id_bank = '$id_bank'");
			if ($q_delete_agama) {
		...
		if ($tb_act == "Tambah") {
			...
			$q_tambah_bank	= mysqli_query($connect,"INSERT INTO tbl_bank VALUES ('', '$p_nm_bank')");
			if ($q_tambah_bank) {
			...
			$display = "style='display: none'";
			$q_edit_statusp	= mysqli_query($connect,"UPDATE tbl_bank SET nm_bank = '$p_nm_bank' WHERE id_bank = '$p_id_bank'");
			if ($q_edit_statusp) {
...
?>
		...
		$q_bank 	= mysqli_query($connect,"SELECT * FROM tbl_bank ORDER BY id_bank ASC") or die (mysqli_error());
		...
		if ($mod == "edit") {
			$display = "";
			$q_edit_bank	= mysqli_query($connect,"SELECT * FROM tbl_bank WHERE id_bank = '$id_bank'");
...
<div class="module width_full" <?php echo $display;?>>
	<header><h3 class="header smaller lighter blue"><?php echo $view;?> Data Bank Transfer</h3></header>
		<div class="module_content">
		<form name="form" action="?id=bank" method="post" id="ft_agama" onSubmit="return validasi(this)">
